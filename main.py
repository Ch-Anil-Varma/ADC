...
async def update_live_leaderboard(question_id, guild):
    ...
    if not subs:
        ...
        for i, sub in enumerate(subs, 1):
            ...
            desc += f"{medal} **{username}** — {sub['score']} pts ({time_str})\n"

    embed = discord.Embed(title=f"📊 Live Leaderboard: {question_data['title']}", description=desc, color=discord.Color.gold())
...
class CodeModal(ui.Modal, title="Submit Your Code"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if is_ai_suspected:
            score = 0
            feedback = "⚠️ **AI Detection Alert:** Your code style strongly resembles AI generation. Score reduced to 0."
        ...
        embed.add_field(name="⏱️ Time Taken", value=f"{mins}m {secs}s", inline=True)
...
class LanguageSelect(ui.Select):
    def __init__(self, q_id, title, desc):
        options = [
            ...
            discord.SelectOption(label="JavaScript", value="JavaScript", emoji="📜"),
            discord.SelectOption(label="C++", value="C++", emoji="⚙️"),
            discord.SelectOption(label="Java", value="Java", emoji="☕"),
        ]
...
async def post(ctx, *, args):
    try:
        ...
        await ctx.send("❌ Usage: `!post Title | Description`")
        return
    ...
    embed = discord.Embed(title=f"📢 New Challenge: {title}", description=description, color=discord.Color.blue())
    embed.add_field(name="⚠️ Anti-Cheat Active", value="Time Limit: 24h\nAI Detection: ON\nSpeed Trap: ON", inline=False)
...
async def global_leaderboard(ctx):
    ...
    for i, user_data in enumerate(top_users, 1):
        ...
        desc += f"**{i}.** {name} — {user_data['score']} pts\n"
